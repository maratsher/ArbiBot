...
class Steps:
    ...
    UPDATE_INIT_VOLUME = 'update_init_volume'
...
async def settings_menu(message: types.Message):
    ...
    kb.add(
        ...
        types.InlineKeyboardButton(bc.UPDATE_INIT_VOLUME, callback_data=f'{MODULE_NAME}:{Steps.UPDATE_INIT_VOLUME}'),
    ...
    settings_info = mc.SETTINGS_INFO.format(
        ...
        init_volume=f"{user.init_volume} {user.target_coin.ticker}",
        volume=f"{user.volume} {user.target_coin.ticker}",
        threshold=f"{user.threshold}",
        epsilon=f"{user.epsilon}",
...
@dp.message_handler(state=SettingsForm.get_init_volume)
async def get_init_volume(message: types.Message, state: FSMContext):
    """
    ...
    if fullmatch(rc.FLOAT_REGEX, volume) and float(volume.replace(',', '.')) <= db_config.MAX_VOLUME:
        ...
        await user_api.update_init_volume(data=schemas.UserVolumeUpdate(telegram_id=str(telegram_id), volume=volume))
        await settings_menu(message=data[StorageDataFields.LAST_MESSAGE])
        await state.finish()
    else:
        text = f"{mc.SETTINGS_TITLE}{mc.LINE}{mc.SETTINGS_INIT_VOLUME}\n\n" \
...
async def callback(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if Steps.BACK in callback_info:
        ...
        elif callback_info == Steps.UPDATE_INIT_VOLUME:
            settings_info = mc.SETTINGS_INIT_VOLUME
            max_value = db_config.MAX_VOLUME
            await SettingsForm.get_init_volume.set()
